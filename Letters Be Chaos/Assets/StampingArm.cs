...
public class StampingArm : MonoBehaviour
{
    ...
    private StampCollisionCheck check;
    ...
    private void Awake()
    {
        ...
        check = transform.Find("art").GetComponent<StampCollisionCheck>();
    }
    ...
    void Update()
    {
        ...
        if (armStatus == ArmStatus.extending)
        {
            ...
            if(Vector2.Distance(art.transform.position, mouseTarget) <= 0.2f)
            {
                ...
                //if letter was stamped
                if (!check.hasHitLetter)
                {
    ...
    {
        AudioManager.Instance.Play("TickHiss");
